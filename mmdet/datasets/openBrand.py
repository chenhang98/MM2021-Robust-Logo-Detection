...
class OpenBrandDataset(CustomDataset):
    ...
    def __init__(self,
        ...
        self.copy_paste_align_rfs = copy_paste_align_rfs
    ...
    def prepare_multiple_train_imgs(self, idx):
        ...
        if self.copy_paste_align_rfs:
            repeat_factors = np.delete(self.repeat_factors, idx)
            selected_indices = random.choice(total_indices, num, False,
                p = repeat_factors / repeat_factors.sum())
        else:
            selected_indices = random.choice(total_indices, num, False)
        ...
        for i in selected_indices:
            ...
            if cnt == 0:
                ...
                extra_aug.append(results_i)
            cnt += 1
        results['extra_aug'] = extra_aug
        return self.pipeline(results)



@DATASETS.register_module()
class MixedOpenBrandDataset(OpenBrandDataset):
    """For each image, random select from img_prefixs[:,1] with prob. img_prefix[:,0].
    # If the filename doesn't exists in an img_prefix, select from others with normalized prob.
    Update: If the filename doesn't exists in an img_prefix, select from candidates[0].
    """
    ...
    def __init__(self,
        ...
        super().__init__(
                 ...
                 copy_paste_align_rfs)
        assert not test_mode, "MixedOpenBrandDataset cannot be used as val/test set."
        self.candidates = candidates    # list of dict(p, img_prefix, signals)
        self._build_fns()               # add `fns` for each candidates

    def _build_fns(self):
        for cand in self.candidates:
            fns = os.listdir(cand['img_prefix'])
            cand['fns'] = set(fns)
    ...
    def pre_pipeline(self, results):
        ...
        if len(self.candidates) > 1:
            probs = [_['p'] for _ in self.candidates]
            if probs[0] is None:
                probs[0] = 1 - sum(probs[1:])
            d = random.choice(len(probs), p=probs)
        else:
            d = 0
        cand = self.candidates[d]       # if not included in this, use candidates[0]
        fn = results['img_info']['filename']
        if fn not in cand['fns']:
            cand = self.candidates[0]
        results['img_prefix'] = cand['img_prefix']
