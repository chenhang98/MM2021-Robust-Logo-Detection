...
class CLAHE:
    def __init__(self,clip_limit=4.0, tile_grid_size=(8, 8), always_apply=False, p=0.5):
        ...
        self.p=p
        self.transform=albumentations.CLAHE(self.clip_limit,self.tile_grid_size,self.always_apply,self.p)

    def __call__(self, results):
        results['img']=self.transform(image=results['img'].astype(np.uint8))['image']
        return results

@PIPELINES.register_module()
class Sharpen:
    def __init__(self,alpha=(0.2, 0.5), lightness=(0.5, 1.0), always_apply=False, p=0.5):
        self.transform=albumentations.IAASharpen(alpha, lightness, always_apply, p)
...
class RandomBrightnessContrast:
    def __init__(self,brightness_limit=0.2, contrast_limit=0.2, brightness_by_max=True, always_apply=False,
                                 p=0.5):
        self.transform=albumentations.RandomBrightnessContrast(brightness_limit, contrast_limit, brightness_by_max, always_apply,
                                 p)
